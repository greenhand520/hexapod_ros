...
namespace ubtech_servo_hardware {
    ...
    public:
        ...
        /// @return true 表示舵机已确认
        bool set_angle(uint8_t id, uint8_t angle_deg,
        ...
        /// @return true 表示读取成功
        bool read_angle(uint8_t id, ServoAngle& angle);
        ...
        bool modify_id(uint8_t old_id, uint8_t new_id);

        /// @brief 设置角度偏移量 (协议: Byte3 = 0xD2)
        /// @param id servo id
        /// @param offset_deg 偏移角度, -30 ~ +30, 正值顺时针
        bool set_offset(uint8_t id, double offset_deg);

        /// @brief 读取角度偏移量设置 (协议: Byte3 = 0xD4)
        bool read_offset(uint8_t id, double & offset_deg);

        /// @brief 读取固件版本号 (协议: Byte3 = 0x01)
        bool read_firmware_version(uint8_t id, std::string & version);

        /// @brief 进入 bootloader (协议: Byte0=FC, Byte1=CF, Byte3=0x02)
        bool enter_bootloader(uint8_t id);
    ...
    private:
        ...
        bool transmit_frame(const uint8_t* frame, size_t len) const;

        // 读取一帧响应 (自动寻找 FA AF 头, 验证校验和与帧尾)
        // @param out      输出缓冲区 (至少 FRAME_LEN 字节)
        // @param timeout  超时 (ms)
        // @return true 表示收到有效帧
        bool receive_frame(uint8_t* out, int timeout_ms = 100) const;
    };
