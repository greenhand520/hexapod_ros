...
#define MPU6500_WHO_AM_I_VALUE          0x70  /* MPU6500 WHO_AM_I 返回值 */


typedef enum {
    MPU6500_ACCEL_RANGE_2G  = 0x00,   /* ±2g,  灵敏度 16384 LSB/g */
    MPU6500_ACCEL_RANGE_4G  = 0x08,   /* ±4g,  灵敏度 8192  LSB/g */
    MPU6500_ACCEL_RANGE_8G  = 0x10,   /* ±8g,  灵敏度 4096  LSB/g */
    MPU6500_ACCEL_RANGE_16G = 0x18    /* ±16g, 灵敏度 2048  LSB/g */
} MPU6500_AccelRange_t;

typedef enum {
    MPU6500_GYRO_RANGE_250DPS  = 0x00,  /* ±250°/s,  灵敏度 131   LSB/(°/s) */
    MPU6500_GYRO_RANGE_500DPS  = 0x08,  /* ±500°/s,  灵敏度 65.5  LSB/(°/s) */
    MPU6500_GYRO_RANGE_1000DPS = 0x10,  /* ±1000°/s, 灵敏度 32.8  LSB/(°/s) */
    MPU6500_GYRO_RANGE_2000DPS = 0x18   /* ±2000°/s, 灵敏度 16.4  LSB/(°/s) */
} MPU6500_GyroRange_t;

typedef enum {
    MPU6500_DLPF_250HZ  = 0x00,  /* 加速度计带宽 250Hz */
    MPU6500_DLPF_184HZ  = 0x01,  /* 加速度计带宽 184Hz */
...
typedef struct {
    ...
    float temperature_c;   /* 芯片温度 (°C) */
} MPU6500_Data_t;

typedef struct {
    SPI_HandleTypeDef *hspi;          /* SPI 句柄 */
    GPIO_TypeDef *CS_GPIO_Port;       /* 片选引脚端口 */
    uint16_t CS_GPIO_Pin;             /* 片选引脚号 */
    MPU6500_AccelRange_t accel_range; /* 加速度计量程 */
    MPU6500_GyroRange_t gyro_range;   /* 陀螺仪量程 */
    MPU6500_DLPF_t dlpf;              /* 低通滤波器 */
    uint16_t sample_rate_divider;     /* 采样率分频器: Sample Rate = 1kHz / (1 + div) */
} MPU6500_Config_t;
